( function( $ ){
    ...
    function addTimeZone($t) {
        if (($t.settings.timeZoneName && $t.settings.timeZoneName.length > 0) &&
            ($t.settings.timeZoneList && $t.settings.timeZoneList.length > 0) &&
            $t.dpDiv.find('.ui_tpicker_timezone_label.ui_tpicker_unit_hide').length === 1) {

            $t.dpDiv.find('.ui_tpicker_timezone_label').removeClass('ui_tpicker_unit_hide');

            var elem = $t.dpDiv.find('.ui_tpicker_timezone').removeClass('ui_tpicker_unit_hide');

            $(elem.children()[0]).remove();
            var child = $('<select></select>').addClass('ui_tpicker_timezone_select');
            elem.append(child);

            var _timeZoneList = $t.settings.timeZoneList.split("|");
            var list = [];
            for (var i = 0, len = _timeZoneList.length; i < len; i += 2) {
                  list.push({ value: _timeZoneList[i], label: _timeZoneList[i] });
            }
            _timeZoneList = list;
            elem = elem.find("select");
            $.fn.append.apply(elem,
            $.map(_timeZoneList, function (val, idx) {
                return $("<option></option>").val(typeof val === "object" ? val.value : val).text(typeof val === "object" ? val.label : val);
            }));
            elem.val($t.settings.timeZoneName);
        }
    };
    ...
    $.datepicker.constructor.prototype._showDatepicker = function(){
        ...
        if( $.datepicker._datepickerShowing ){
            ...
            inst.dpDiv.attr("tabindex", "-1");
            addTimeZone.call(this, inst);
            ...
            inst.dpDiv
                ...
                .bind( "keydown.closePicker" ,{ "inst": inst }, function( evt ){
                    ...
                    switch( evt.which ){
                        case 9: // hide on Tab
                                  if ( !evt.shiftKey){
                                      inst.dpDiv.find(".ui-datepicker-prev").focus();
                                      evt.preventDefault();
                                  }
                                  break;
                ...
                .bind( "keydown.prevButton", { "inst": inst }, function( evt ) {
                    ...
                    switch( evt.which ) {
                        case 9:   // hide on Shift Tab
                                  if ( evt.shiftKey){
                                      inst.dpDiv.find(".ui-datepicker-close").focus();
                                      evt.preventDefault();
                                  }
                                  break;
            ...
            inst.dpDiv
                .off( "keydown.dayDiv",  ".ui-datepicker-calendar", false )
                .on("keydown.dayDiv", ".ui-datepicker-calendar", { "inst": inst }, function (evt) {
    ...
    $.datepicker._updateDatepicker = function(){
            ...
            .find( "table.ui-datepicker-calendar" )
                ...
                .children( "tbody" )
                    ...
                    .find( "td" )
                        ...
                        .children()
                            .attr("tabindex", "-1");
        inst.dpDiv.find(".ui-datepicker-unselectable").removeAttr("id");
        inst.dpDiv.find(".ui-datepicker-unselectable").removeAttr("id")
        inst.dpDiv
        ...
        inst.dpDiv
            ...
            .attr("tabindex", "0").off("change.onSelectYear").on("change.onSelectYear", { ctx: inst }, function (evt) {
                var inst = evt.data.ctx;
                inst.dpDiv
                    .find(".ui-datepicker-year")
                    .attr("tabindex", "0")
                    .focus();
        ...
        inst.dpDiv
            ...
            .attr("tabindex", "0").off("change.onSelectMonth").on("change.onSelectMonth", { ctx: inst }, function (evt) {
                var inst = evt.data.ctx;
                inst.dpDiv
                    .find(".ui-datepicker-month")
                    .attr("tabindex", "0")
    ...
    var SWE__attachHandlers = $.datepicker.constructor.prototype._attachHandlers;
    $.datepicker.constructor.prototype._attachHandlers = function () {
        var returnValue = SWE__attachHandlers.apply(this, arguments);
        var inst = arguments[0];
        var id = "#" + inst.id.replace(/\\\\/g, "\\");
        inst.dpDiv.find("[data-handler]").map(function () {
            var handler = {
                prev: function () {
                    $.datepicker._clearSelection(id);
                },
                next: function () {
                    $.datepicker._clearSelection(id);
                },
                selectMonth: function () {
                    $(this).trigger("change.onSelectMonth");
                    $.datepicker._clearSelection(id);

                },
                selectYear: function () {
                    $(this).trigger("change.onSelectYear");
                    $.datepicker._clearSelection(id);
                }
            };
    ...
    $.datepicker.constructor.prototype._clearSelection = function (eleId) {
        var target = $(eleId),
            inst = this._getInst(target[0]);
        var id = "ui-datepicker-day-" + inst.selectedDay;
        var elem = $("#" + id);
        elem.children("a")
                 .removeClass("ui-state-hover");
    }

    $.datepicker.SWE__base_selectDate = $.datepicker._selectDate;
    $.datepicker._selectDate = function (id, dateStr) {
        var inst = this._getInst($(id)[0]);
        this.SWE__base_selectDate(id, dateStr);
        if (!inst.input.is(":focusable")) {
           $(id).next().focus();
        }
    }
