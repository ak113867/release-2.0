...
$.extend($.jgrid,{
	...
	msie11: !!navigator.userAgent.match(/Trident/),
	msiever : function () {
		var rv = -1;
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
	    ...
	    var gv = $("<div class='ui-jqgrid-view' id='gview_"+this.id+"' ></div>"),
		isMSIE = $.jgrid.msie;
		...
		var eg = $("<div class='ui-jqgrid ui-widget ui-widget-content ui-corner-all' id='gbox_"+ this.id +"' dir='" + dir + "' ></div>");
		eg.insertBefore(gv);
		gv.appendTo(eg);
		$("<div class='ui-widget-overlay jqgrid-overlay' id='lui_"+this.id+"'></div>" +
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			if( data.records && !ts.p.treeGrid && !ts.p.scroll) {ts.grid.bDiv.scrollTop = 0;}
			ts.p.reccount=ir;
			ts.p.treeANode = -1;
			if(ts.p.userDataOnFooter) { self.jqGrid("footerData","set",ts.p.userData,true); }
			if(locdata) {
			...
			if ( data.records && !more) { ts.updatepager(false,true); }
			if(locdata) {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if(ts.p.pgbuttons===true) {
					if(cp<=0) {cp = last = 0;}
					if(cp===1 || cp === 0) {
					    $("#first" + tspg ).addClass('ui-state-disabled').removeClass('ui-state-hover');
					    $("#prev" + tspg).addClass('ui-state-disabled').removeClass('ui-state-hover');
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-state-hover'); }
					} else {
					    $("#first" + tspg).removeClass('ui-state-disabled');
					    $("#prev" + tspg).removeClass('ui-state-disabled');
						if(ts.p.toppager) { $("#first_t"+tspg_t+", #prev_t"+tspg_t).removeClass('ui-state-disabled'); }
					}
					if(cp===last || cp === 0) {
					    $("#next" + tspg ).addClass('ui-state-disabled').removeClass('ui-state-hover');
					    $("#last" + tspg).addClass('ui-state-disabled').removeClass('ui-state-hover');
						if(ts.p.toppager) { $("#next_t"+tspg_t+", #last_t"+tspg_t).addClass('ui-state-disabled').removeClass('ui-state-hover'); }
					} else {
					    $("#next" + tspg).removeClass('ui-state-disabled');
					    $("#last" + tspg).removeClass('ui-state-disabled');
		...
		if (ts.p.autowidth === true) {
		    //[Perf-OpenUI - Need Replacement 
		    var pw = ts.p.initWidth || $this.parent().data("outerWidth");//$(eg).innerWidth();
		...
		for(i=0;i<this.p.colNames.length;i++){
		    ...
		    if(ts.p.colModel[i].resizable === undefined) {ts.p.colModel[i].resizable = true;}
			var tooltip = ts.p.headertitles ? (" title=\""+$.jgrid.stripHtml(ts.p.colNames[i])+"\"") :"";
			thead += "<th id='" + ts.p.id + "_" + ts.p.colModel[i].name + "' role='columnheader' class='ui-state-default ui-th-column ui-th-" + dir + " " + ( ts.p.colModel[i].resizable ? ts.p.resizeclass : "" ) + "' " + tooltip + " style='width:" + ts.p.colModel[i].width + "px;'>";
			idn = ts.p.colModel[i].index || ts.p.colModel[i].name;
			if (ts.p.colModel[i].resizable === undefined || ts.p.colModel[i].resizable) {
			    thead += "<span class='ui-jqgrid-resize ui-jqgrid-resize-" + dir + "' style='cursor:col-resize;' >&#160;</span>";
			}

			sort = ts.p.colModel[i].sortable;
			if (typeof sort !== 'boolean') { ts.p.colModel[i].sortable = true; sort = true; }
			var nm = ts.p.colModel[i].name;
			if (!(nm === 'cb' || nm === 'subgrid' || nm === 'rn')) {
			    if (ts.p.viewsortcols[2]) {
			        tdc = isMSIE ? "class='ui-th-div-ie ui-jqgrid-sortable'" : "class='ui-jqgrid-sortable'";
			    }
			}
...
$.jgrid.extend({
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			if($t.p.tblwidth) {
				...
				var selfEl = $t;
				fastdom.read(function () {
				    var _val_ = selfEl.grid.hDiv.scrollLeft;
				    fastdom.write(function () {
				        selfEl.grid.bDiv.scrollLeft = _val_;
				        selfEl = null;
				    })
				});
	...
	setLabel : function(colname, nData, prop, attrp ){
		return this.each(function(){
			...
			if(pos>=0) {
			    var thecol = $(".ui-jqgrid-labels",$t.grid.hDiv).find( "th" ).eq( pos )
				if (nData){
	...
	setCell : function(rowid,colname,nData,cssp,attrp, forceupd) {
		return this.each(function(){
			...
			if(pos>=0) {
				...
				if (ind){
                    var tcell = $(ind).find( "td" ).eq( pos ); //$("td:eq("+pos+")",ind);
	...
	getCell : function(rowid,col) {
		...
		this.each(function(){
			...
			if(pos>=0) {
				...
				if(ind) {
					try {
                        ret = $.unformat.call($t,$(ind).find("td").eq(pos),{rowId:ind.id, colModel:$t.p.colModel[pos]},pos);
					} catch (e){
                        ret = $.jgrid.htmlDecode($(ind).find("td").eq(pos).html());
					}
				}
			}
		});
	...
	clearGridData : function(clearfooter) {
		return this.each(function(){
            ...
            tbodyFirst = $("#"+$.jgrid.jqID($t.p.id)+" tbody:first");
            if($t.p.deepempty) {tbodyFirst.find("tr:gt(0)").remove();}
			else {
                var trf = tbodyFirst.find("tr:first")[0];
                tbodyFirst.empty().append(trf);
			}
