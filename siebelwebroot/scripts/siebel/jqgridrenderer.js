...
if (typeof (SiebelAppFacade.JQGridRenderer) === "undefined") {
    ...
    define("siebel/jqgridrenderer", ["3rdParty/jqGrid/current/js/i18n/grid.locale-en.js", "3rdParty/jqGrid/current/js/jquery.jqGrid.min.js", "3rdParty/jqgrid-ext.js", "siebel/phyrenderer"],
      function () {
          SiebelAppFacade.JQGridRenderer = (function () {
              ...
              var RN = siebConsts.get("SWE_PROP_AUTOM_RN");
              var OT = siebConsts.get("SWE_PROP_AUTOM_OT");
              var UN = siebConsts.get("SWE_PROP_AUTOM_UN");
              ...
              JQGridRenderer.prototype.Init = function () {
                  ...
                  this.AttachPMBinding("MultiSelectMode", function (name,val) {
                  ...
                  this.AttachPMBinding("InvokeStateChange", UpdatePagerControls);
                  this.AttachPMBinding("OnScrollComplete", OnScrollComplete);

                  this.AttachPMBinding("OnRecNavComplete", OnRecNavComplete);

                  this.AttachPMBinding("ForceFocusOnControl", ForceFocusOnControl);

                  var wTRecordsHO = SiebelApp.S_App.PluginBuilder.GetHoByName("WTRecordsHO");
                  var dyListHO = SiebelApp.S_App.PluginBuilder.GetHoByName("DynamicListHO");
                  if (wTRecordsHO) {
                      wTRecordsHO.SetUp("", this);
                  }
                  if (dyListHO && this.GetPM().ExecuteMethod("CanInvokeMethod", "RefreshRowCount")) {
                      dyListHO.SetUp(this);
                  }
              };

              function OnRecNavComplete(direction) {
                  var pm = this.GetPM(),
                  self = this;
                  setTimeout(function () {
                      if (pm && pm.Get("IsRecNavOn")) {
                          pm.OnControlEvent(siebConsts.get("PHYEVENT_VSCROLL_LIST"), direction);
                      }
              ...
              JQGridRenderer.prototype.MaskLeaveField = function (ctrl, bLeaveFieldState) {
                  ...
                  if (element.length > 0) {
                      ...
                      SiebelAppFacade.JQGridRenderer.superclass.MaskLeaveField.call(this, ctrl, bLeaveFieldState);
                  }
              };
              ...
              JQGridRenderer.prototype.EndLife = function () {
                  ...
                  $(window).off("resize.JQGrid"+pm.Get("GetFullId"));
              ...
              function drilldownFormatter(cellvalue, options, rowObject) {
                  ...
                  if (cellvalue) {
                      ...
                      var ctrlName = this.GetColumnHelper().GetActualControlName(options.colModel.name),
                          ...
                          : "<a  href=\"javascript:void(0);\" class=\"drilldown\" name=\"" +
                              ...
                              if (IconMapArray[i].iconName == cellvalue) {
                                  ...
                                  if (!formatedString) {
                                      ...
                                      if (utils.IsTrue(SiebelApp.S_App.IsAutoOn()) && !utils.IsEmpty(iconName)) {
                                          attrObj[consts.get("SWE_PROP_AUTOM_OT")] = consts.get("SWE_PROP_ICON_MAP");
                                          attrObj[consts.get("SWE_PROP_AUTOM_RN")] = iconName;
                                          attrObj[consts.get("SWE_PROP_AUTOM_UN")] = cellvalue;
                                      }
                                      iconEl.attr(attrObj);
              ...
              function phoneFieldFormatter(cellvalue, options, rowObject) {
                  ...
                  return "<a" + href + " class=\"" + className + "\" name=\"" + this.GetColumnHelper().GetActualControlName(options.colModel.name) + "\" tabindex=\"-1\" >" + HtmlEncode(cellvalue) + "</a>";
              };
              ...
              function PrepareGroupedColumnInfo(listOfColumns) {
                ...
                function PrepareEachColumnInfo(currentColumn){
                  ...
                  //Add the Column with FieldName as the key
                      ...
                      // Bug 25479667 - VALUES IN PROBABILITY % BLANKS OUT WHEN CLICKED ON IT IN FIREFOX
                      if (current.align === "center") {
                        current.classes = "force-overflow-visible";
                      }
                      switch (currentColumn.controlType) {
                          ...
                          case checkbox:
                              ...
                              current.editoptions.custom_value = function (elem, operation, value) {
                                  ...
                                  if (operation === 'get') {
                                      if (el.is(":checkbox")) {
                                          if (utils.IsTrue(el.prop("indeterminate"))) {
                                              el.val("");
                                          }
                                          else {
                                              el.val(utils.IsTrue(el.prop("checked")) ? "Y" : "N");
                                          }
                                      }
                                      return el.val();
                                  }
                              ...
                              if (currentColumn.name === pm.Get("MultiSelectColumn")) {
                                  ...
                                  current.formatter = function (value, options, rowObject) {
                                      var inputtype = "checkbox",
                                          AUTOM_OT = consts.get("SWE_PROP_AUTOM_OT"),
                                          AUTOM_RN = consts.get("SWE_PROP_AUTOM_RN"),
                                          name = that.GetPM().Get("GetPlaceholder") + that.GetPM().Get("MultiSelectColumn");
                                      inputtype = getCheckboxtype.call(that, options.rowId);
                                      var cb = ("<input type=\"" + inputtype + "\" name = \"" + name + "\" role=\"checkbox\" tabindex=\"0\" value=\"" + value + "\" aria-checked= " + (value === "Y" ? '"true"' : '"false"') + " " + (value === "Y" ? "checked=checked" : "") +
                                          " " + (utils.IsTrue(SiebelApp.S_App.IsAutoOn()) ? consts.get("SWE_PROP_AUTOM_RN") + "=\"" + that.GetPM().Get("MultiSelectColumn") + "\" " +
                                          consts.get("SWE_PROP_AUTOM_OT") + "=\"" + checkbox + "\"" : "") + " />");
                                      return cb;
                                  };
                      ...
                      current.cellattr = function (rowId, rowValue, rawObject, cm, rdata, pos) {
                          if(pos !== undefined) {
                              return (that.AddAccessibilityInfo(cm.controlType, cm.isLink, rdata[cm.name], pos) + ' id=' + GetColId(rowId, this.id, cm.name));
                          }
                      };
                      ...
                      var colname = HtmlEncode(currentControl.GetDisplayName()),
                          ...
                          multiSel = (SiebelApp.S_App.LocaleObject.GetLocalString("IDS_SWE_CKEDITOR_SELECT_ALL") ? "Select All" : ""),
                          rnString = "";
                      if (utils.IsTrue(isAutoOn)) {
                          rnString = "ot = \"" + checkbox + "\" rn = \"" + mSelCol + "\" un = \"" + mSelCol + "\"";
                      }
              ...
              function GetColId(rowId, ctxId, ctrlName) {
                return ("" + rowId + "_" + ctxId + "_" + ctrlName);
              }
              ...
              *  */
              ...
              JQGridRenderer.prototype.ShowUI = function () {
                  ...
                  if (jqGridCtrl.length > 0) {
                      ...
                      try {
                          ...
                          var bIsSmallScreen = SiebelApp.Utils.IsSmallScreen();
                          if (hasHierarchy) {
                              $.extend(gridConfig, {
                                  gridview: false,
                                  treeGrid: true,
                                  ...
                                  treeIcons: {
                                      plus: bIsSmallScreen ? "" : 'ui-icon-triangle-1-e',
                                      minus:  bIsSmallScreen ? "" : 'ui-icon-triangle-1-s',
                          ...
                          if(bIsSmallScreen){
                              $("#s_" + pm.Get("GetFullId") + "_div").find("span.s-ico").find("span.ui-icon-triangle-1-s").removeClass("ui-icon-triangle-1-s");
                          }
                          if (lastsort) {
                          ...
                          var pagerDiv = $("#pager_" + placeHolder);
                          // Why JQGrid does not provide a way to set Tooltip? Also i18n?
                          // Before Changing tooltip, make sure to change in OnPagination
                          var nextPg = pagerDiv.find("#next_pager_" + placeHolder).children("span").attr("title", localeObj.GetLocalString("NextRecSetToolTip"));
                          var lastPg = pagerDiv.find("#last_pager_" + placeHolder).children("span").attr("title", localeObj.GetLocalString("LastRecSetToolTip"));
                          var prevPg = pagerDiv.find("#prev_pager_" + placeHolder).children("span").attr("title", localeObj.GetLocalString("PrevRecSetToolTip"));
                          var firstPg = pagerDiv.find("#first_pager_" + placeHolder).children("span").attr("title", localeObj.GetLocalString("FirstRecSetToolTip"));

                          
                          if (SiebelApp.S_App.GetDirection()) {                             
                              pagerDiv.find('.ui-icon-seek-first').switchClass("ui-icon-seek-first", "ui-icon-seek-end");
                              pagerDiv.find('.ui-icon-seek-prev').switchClass("ui-icon-seek-prev", 'ui-icon-seek-next');
                              pagerDiv.find('.ui-icon-seek-next').last().switchClass("ui-icon-seek-next", 'ui-icon-seek-prev');
                              pagerDiv.find('.ui-icon-seek-end').last().switchClass("ui-icon-seek-end", 'ui-icon-seek-first');                            
                          }

                          if (isAutoOn) {//Automation attributes
                              nextPg.attr({
                                  OT: "button",
                                  RN: "Next Record Set",
                                  UN: localeObj.GetLocalString("NextRecSetToolTip")
                              });
                              lastPg.attr({
                                  OT: "button",
                                  RN: "Last Record Set",
                                  UN: localeObj.GetLocalString("LastRecSetToolTip")
                              });
                              prevPg.attr({
                                  OT: "button",
                                  RN: "Previous Record Set",
                                  UN: localeObj.GetLocalString("PrevRecSetToolTip")
                              });
                              firstPg.attr({
                                  OT: "button",
                                  RN: "First Record Set",
                                  UN: localeObj.GetLocalString("FirstRecSetToolTip")
                              });

                          }

                          pagerDiv.addClass("siebui-applet-footer");
                          pagerDiv = nextPg = lastPg = prevPg = firstPg = null;
                          ...
                          for (colIndex = 0; colIndex < colModelLength; colIndex++) {
                              ...
                              if (isAutoOn) {
                                  ...
                                  if (pm.Get("MultiSelectColumn") === currentCol.name) {
                                      target.attr({
                                          RN : currentCol.name,
                                          OT : checkbox,
                                          UN : currentCol.name
                                  ...
                                  SiebelAppFacade.JQGridRenderer.superclass.InjectAutomationAttr.call(this, target, this.GetColumnControl(currentCol.name));
                                  var IsReadOnly = !utils.IsTrue(pm.Get("IsEditable") === "1") || !currentCol.editable;

                                  if (IsReadOnly) {
                                      target.attr("data-readonly", "readonly");
                                  }
                                  if (currentCol.name === "title"){//jqgrid bug
                                      target.attr(RN, target.attr(RN).replace(/title/i, 'title'));
                                      target.attr(UN, target.attr(UN).replace(/title/i, 'title'));
                                  }
                                  
                              }
                              if (SiebelApp.S_App.IsConfigMode()){
                                    var parent_target = $(temp + "_" + currentCol.name).parent();
                                    SiebelAppFacade.JQGridRenderer.superclass.InjectConfigInfo.call(this, parent_target, this.GetColumnControl(currentCol.name),"AWTI");
                              }
                          ...
                          var headerMenuId = "s_" + pm.Get("GetFullId") + "_headerMenu",
                               ...
                               headerMenuHTML = "<ul id='" + headerMenuId + "' class='siebui-list-header-menu' ot='headerMenu' role='menu' un='headerMenu' rn='headerMenu'>"
                          headerMenuHTML += "<li id=" + "'" + "SortAsc_" + pm.Get("GetId") + "' class='siebui-list-header-menu-item' ot='sortColumn' rn='sortColumn-asc' un='sortColumn' role='presentation'><a href='javascript:void(0)' " + "aria-label='" + strSortAscToolTip + "' title='" + strSortAscToolTip + "' role='menuitem' class='siebui-icon-arrowsm-up'></a></li>"
                          headerMenuHTML += "<li id=" + "'" + "SortDesc_" + pm.Get("GetId") + "' class='siebui-list-header-menu-item' ot='sortColumn' rn='sortColumn-desc' un='sortColumn' role='presentation'><a href='javascript:void(0)' " + "aria-label='" + strSortDesToolTip + "' title='" + strSortDesToolTip + "' role='menuitem' class='siebui-icon-arrowsm-down'></a></li>"
                          headerMenuHTML += "<li id=" + "'" + "Lock_" + pm.Get("GetId") + "' class='siebui-list-header-menu-item' ot='lockColumn' rn='lockColumn' un='lockColumn' role='presentation'><a href='javascript:void(0)' " + "aria-label='" + strLock + "' title='" + strLock + "' role='menuitem' class='siebui-icon-lock'></a></li>"
                          headerMenuHTML += "<li id=" + "'" + "Unlock_" + pm.Get("GetId") + "' class='siebui-list-header-menu-item' ot='unlockColumn' rn='unlockColumn' un='unlockColumn' role='presentation'><a href='javascript:void(0)' " + "aria-label='" + strUnlock + "' title='" + strUnlock + "' role='menuitem' class='siebui-icon-unlock'></a></li>"
                               headerMenuHTML += "</ul>";
                          ...
                          if (thd.length) {
                              ...
                              var trEl = thd.find("tr.ui-jqgrid-labels");
                              thd.attr("role", "rowgroup");
                              trEl.attr("role", "row");
              ...
              function DisplaySortInfo(control, direction) {
                  ...
                  if (control) {
                      if (control === "reset") {
                          var sortedCol = grid.jqGrid("getGridParam", "sortname");
                          if (sortedCol) {
                          ...
                          var sortColEle = $("#jqgh_" + this.GetPM().Get("GetPlaceholder") + "_" + modifiedCtrlName);
                          if (modifiedCtrlName !== grid.jqGrid("getGridParam", "sortname")) {
                              $("#jqgh_" + this.GetPM().Get("GetPlaceholder") + "_" + grid.jqGrid("getGridParam", "sortname")).find("span.s-ico").styleHide();
                              grid.jqGrid("setGridParam", { 'sortname': modifiedCtrlName, 'sortorder': direction });
                              (direction === "desc") ? sortColEle.find(".ui-grid-ico-sort.ui-icon-asc").styleHide() : sortColEle.find(".ui-grid-ico-sort.ui-icon-desc").styleHide();
                          }
                          if (modifiedCtrlName) {
                          ...
                          if (listColLength > 0) {
                              ...
                              for (colIndex = 0; colIndex < listColLength; colIndex++) {
                                  ...
                                  if (columnControl.GetFieldName()) {
                                      var emptyEl = recordSetLength > 0 ? $('[name="' + columnControl.GetInputName() + '"]').slice(recordSetLength) : $('[name="' + columnControl.GetInputName().replace("-1", "65535") + '"]');
                                      (SiebelApp.S_App.IsRwd() && (utils.IsTrue(pm.Get("IsObjResponsive")))) ? emptyEl.closest("div").styleHide() : emptyEl.closest("tr").styleHide();
                                  }
                              }
                              ...
                              for (var recordIndex = 0; (recordIndex < recordSetLength) ; recordIndex++) {
                                  ...
                                  for (colIndex = 0; colIndex < listColLength; colIndex++) {
                                      ...
                                      if (columnControl.GetFieldName()) {
                                          ...
                                          var ctrlCntnr = $('[name="' + columnControl.GetInputName() + '"]').eq(recordIndex);
                                          (SiebelApp.S_App.IsRwd() && (utils.IsTrue(pm.Get("IsObjResponsive")))) ? ctrlCntnr.closest("div").styleShow() : ctrlCntnr.closest("tr").styleShow();
              ...
              function AddDragandDropCapabilty(pm) {
                  ...
                  $("#" + pm.Get("GetPlaceholder")).find("tr").draggable({
                      ...
                      start: function (evt, ui) {
                          ...
                          if (useDragSrc) {
                              if (utils.IsTrue(SiebelApp.S_App.IsAutoOn())) {
                                  SiebelApp.EventManager.fireEvent("CustomControlEvent:Change", evt.originalEvent);
                              };
                      ...
                      stop: function () {
                          $(".ui-draggable-dragging").html("").remove(); // Remove previously dragging items
              ...
              JQGridRenderer.prototype.AddAccessibilityTags = function () {
                  ...
                  var htmlString = "<span id='" + placeHolder + "_altpick'          aria-label='" + localeObj.GetLocalString("IDS_SWE_PICK_TITLE") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altmvg'           aria-label='" + localeObj.GetLocalString("IDS_SWE_MVG_TITLE") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altCurrCalc'      aria-label='" + localeObj.GetLocalString("IDS_SWE_CURRENCY_CALC_FIELD_TITLE") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altCalc'          aria-label='" + localeObj.GetLocalString("IDS_SWE_CALC_FIELD_TITLE") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altDate'          aria-label='" + localeObj.GetLocalString("IDS_SWE_DATE_FIELD_TITLE") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altDateTime'      aria-label='" + localeObj.GetLocalString("IDS_SWE_DATETIME_FIELD_TITLE") + "' ></span> " +
                                   "<span id='" + placeHolder + "_altDateTimeZone'  aria-label='" + localeObj.GetLocalString("IDS_SWE_DATETIMEZONE_FIELD_TITLE") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altEffDate'       aria-label='" + localeObj.GetLocalString("IDS_SWE_EFFECTIVE_DATE_TITLE") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altCombo'         aria-label='" + localeObj.GetLocalString("IDS_SWE_COMBO_FIELD") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altLink'          aria-label='" + localeObj.GetLocalString("IDS_SWE_CKEDITOR_LINK") + "' > </span> " +
                                   "<span id='" + placeHolder + "_altCheckBox'      aria-label='" + localeObj.GetLocalString("IDS_SWE_CKEDITOR_CHECKBOX") + "' > </span> ";
              ...
              JQGridRenderer.prototype.AddAccessibilityInfo = function (controlType, isLink, val, index) {
                  var altText = "",
                      descby ="",
                  ...
                  if (descby) {
                      retval += " aria-labelledby = " + "\"" + descby.trim() + "\" ";
                  }

                  var selectAllCheckbox = this.GetPM().Get("GetPlaceholder") + this.GetPM().Get("MultiSelectColumn");
                  if (this.GetColName(index).indexOf(selectAllCheckbox) === -1) {
                      retval += " aria-roledescription = " + "\"" + this.GetColName(index) + "\"";
                  }
                  else {
                      // not adding for multi select checkbox ...
                  }
              ...
              JQGridRenderer.prototype.PostBindData = function () {
                  ...
                  DOMUtils.Read(function () {
                      ...
                      DOMUtils.Write(function () {
                          ...
                          if (SiebelAppFacade.DecisionManager.IsTouch()) {
                              var recordCount = Number(pm.Get("GetRecordSet").length);
                              var listRowCount = Number(pm.Get("GetRowListRowCount"));
                              if (recordCount > listRowCount / 3) {
              ...
              function UpdatePagerControls () {
                  var pm = this.GetPM(),
                  placeHolder = pm.Get("GetPlaceholder"),
                  pgCtrls = [ {"ctrlElem" : $("#first_pager_" + placeHolder) , "methodName" : "GotoFirstSet"} ,
                              {"ctrlElem" : $("#prev_pager_" + placeHolder) , "methodName" : "GotoPreviousSet"} ,
                              {"ctrlElem" : $("#next_pager_" + placeHolder) , "methodName" : "GotoNextSet"} ,
                              {"ctrlElem" : $("#last_pager_" + placeHolder) , "methodName" : "GotoLastSet"} ],
                  len = pgCtrls.length,
                  i;
                  for(i = 0; i < len; i++) {                   
                      if( pm.ExecuteMethod( "CanInvokeMethod", pgCtrls[ i ].methodName )){
                          pgCtrls[i].ctrlElem.removeClass( "ui-state-disabled" ).children( "span").attr( "tabindex" , "0" );
                      }
                      else {
                          pgCtrls[i].ctrlElem.addClass("ui-state-disabled").children("span").removeAttr("tabindex");
                      }                     
                  }
              }
                  ...
                  if (SiebelAppFacade.DecisionManager.IsTouch()) {
                      if (recCount) {
                          ...
                          if (iscroll) {
                              ...
                              var placeHolder = pm.Get("GetPlaceholder");
                              if (jqGridCtrl.parent().attr('id') !== placeHolder + "_scroll") {
                              ...
                              $("#" + placeHolder + "_scroll").on('touchstart', { ctx: this }, function (evt) {
                                  var self = evt.data.ctx;
                                  self.GetPM().SetProperty("isTouchScroll", true);
                                  if (!self.GetGrid().data("Iscroll")) {
                                      InitScroll.call(self, true, true, false);
                                  }
                              });
                              $("#" + placeHolder + "_scroll").on('wheel', { ctx: this }, function (evt) {
                                  var self = evt.data.ctx;
                                  self.GetPM().SetProperty("isTouchScroll", false);
                                  if(evt.shiftKey){
                                      if (!self.GetGrid().data("Iscroll")) {
                                          InitScroll.call(self, true, true, false);
                                      }
                                      ...
                                      if (self.GetGrid().data("Iscroll")) {
                                          self.GetGrid().data("Iscroll").destroy();
                                          self.GetGrid().removeData("Iscroll");
                                      }
                      ...
                      if (recCount) {
                          ...
                          if (!iscroll) {
                              if (jqGridCtrl.parent().attr('id') !== placeHolder + "_scroll") {
                                  jqGridCtrl.data("recCount", recCount).wrapAll(
                                                  "<div id=\"" + placeHolder + "_scroll\">");
                              }
                              jqGridCtrl.parent().css("height", jqGridCtrl.height() - 1);
                              iscroll = new IScroll("#" + placeHolder + "_scroll", {
                                  infiniteElements: jqGridCtrl,
                                  infiniteLimit: 1,
                                  disablePointer: true,
                              ...
                              iscroll.on("scrollEnd", scrollNextPrev);
                              $("#" + placeHolder + "_scroll").on('wheel', { ctx: this }, function (evt) {
                                  var self = evt.data.ctx;
                                  if(evt.shiftKey){
                                     self.GetPM().SetProperty("metaScroll", true);
                                     InitScroll.call(self, true, true, false);
                                  }
                                  else {
                                      if (self.GetGrid().data("Iscroll")) {
                                     ...
                                     self.GetPM().SetProperty("metaScroll", false);
                                  }
                              });
                              jqGridCtrl.data("Iscroll", iscroll);
                          }
                          else if(resetScroll) {
                              refreshIscroll.call(this);
                          }
                      }
                      else {
                          this.GetGrid().parent().css("height", "auto");
                      }
                  }
              }

              function scrollNextPrev() {
                  var direction = this.directionY,
                  pr = this.options.ctx,
                  placeHolder = pr.GetPM().Get("GetPlaceholder"),
                  grid = pr.GetGrid();
                  if(pr.GetPM().Get("metaScroll")){
                    if (!grid.data("ScrollPending")) {
                      if (direction > 0 && !$("#next_pager_" + placeHolder).hasClass("ui-state-disabled")) {
                          grid.data("ScrollPending", true);
                          pr.OnPagination("next_pager");
                      }
                      else if (direction < 0 && !$("#prev_pager_" + placeHolder).hasClass("ui-state-disabled")) {
                          grid.data("ScrollPending", true);
                          pr.OnPagination("prev_pager");
                      }
                  }
                  grid = null;
               }
                  ...
                  if (SiebelAppFacade.DecisionManager.IsTouch() && scrollToEl && scrollToEl.length && iscroll) {
                      ...
                      scrollTimer = setTimeout(function () {
                          if ((scrollToElOffset < scrollContainerOffset) || (scrollToElOffset > (scrollContainerOffset + scrollContainerHeight - scrollToElHeight))) {
                              iscroll.scrollToElement(scrollToEl.parents("tr")[0], easeTime, 0, false);
                          }
                          scrollTimer = 0;
                      }, 0);
                  }
                  ...
                  if (pm.OnControlEvent(siebConsts.get("PHYEVENT_APPLET_FOCUS"))) {
                      ...
                      if ($(this).prop("checked") === true) {
                          ...
                          if (pm.ExecuteMethod("CanInvokeMethod", "SelectAll")) {
                            ...
                            ctx.GetGrid().resetSelection();
                            ctx.CacheState("SelectedRowArray", []);
                            ctx.OnRowSelect(1, e.ctrlKey, e.shiftKey);
                      }
                  }
              }
              ...
              JQGridRenderer.prototype.BindEvents = function (controlSet) {
                  ...
                  if (grid.length === 1) {
                      ...
                      $("#s_" + this.GetPM().Get("GetFullId") + "_div").find(".ui-jqgrid-htable th.ui-th-column").bind("click", { ctx: this }, function (evt) {
                          var self = evt.data.ctx;
                          var pm = self.GetPM();
                          if (self && pm && !pm.Get("IsActive")) {

                              var focusApplet = pm.OnControlEvent(siebConsts.get("PHYEVENT_APPLET_FOCUS"));
                              if (!focusApplet) {
                                  $(document).triggerHandler("mousedown.HideHeaderMenu");
                              }
                          }
                      });
                  ...
                  var navCtrls = $("#next_pager_" + placeHolder +
                    ", #last_pager_" + placeHolder +
                    ", #prev_pager_" + placeHolder +
                    ", #first_pager_" + placeHolder),
                    pr = this;

                  navCtrls.on("touchend", function () {
                         $(this).removeClass("ui-state-hover");
                     }).off("click");
                  navCtrls.children("span").each(function(){
                      $(this).on("mousedown keydown", { ctx: pr}, function (evt) {
                          if (evt.which === $.ui.keyCode.ENTER || evt.type === "mousedown") {
                          var prObj = evt.data.ctx,
                          pmObj = prObj.GetPM();
                          if (utils.IsTrue(SiebelApp.S_App.IsAutoOn())) {
                              SiebelApp.UserActionRecorder.CreateEvtObj("explicitClick", $(this),"" ,"", "", "", "");
                          }
                          if (evt.type === "mousedown") {
                              pmObj.SetProperty("IsRecNavOn", true);
                              $(this).addClass("siebui-mouse-down");
                          }
                          prObj.OnPagination(
                                $(this).parent().attr("id").replace("_" + pmObj.Get("GetPlaceholder"), ""));
                          if(evt.type === "mousedown") {
                              $(document).on('mouseup', { ctx: pr, target: this, isMouseUp: true }, stopRecNav);
                              $(this).data("pos", { "x": evt.clientX, "y": evt.clientY });
                          }
                     } }).on("mouseleave", { ctx: pr, target: this}, stopRecNav);
                  });
                  function stopRecNav (evt) {
                      var pos = $(evt.data.target).data("pos");
                      if(evt.data.isMouseUp || (pos && evt.clientX != pos.x && evt.clientY != pos.y)) {
                          $(evt.data.target).removeClass("siebui-mouse-down");
                          evt.data.ctx.GetPM().SetProperty("IsRecNavOn", false);
                      }
                      if(evt.data.isMouseUp) {
                          $(this).off('mouseup', stopRecNav);
                      }
                  }
                  // Column sort feature.
                  ...
                  /* Custom Events */
                  $(window).on("resize.JQGrid"+pm.Get("GetFullId"), { ctx: this }, function (event) {
              ...
              function GridShowHeaderMenu(index, iCol, sortorder) {
                  ...
                  if (self) {
                      ...
                      if (index !== pm.Get("JQGridProp_lastsortCol") && !self.GetPM().Get("SortMode")) {
                          $("#s_" + pm.Get("GetFullId") + "_div").find(".siebui-icon-arrowsm-sort").attr("rn", "sortColumn-asc");
                      }
                      if (!self.GetPM().Get("SortMode")) {
                          ...
                          if(iCol === pm.Get("JQGridProp_lastsort")){
                              if(sortOrder === "asc"){
                                  headerMenu.find("#SortAsc_" + pm.Get("GetId")).hide();
                                  headerMenu.find("#SortDesc_" + pm.Get("GetId")).show();
                              }else{
                                  headerMenu.find("#SortDesc_" + pm.Get("GetId")).hide();
                                  headerMenu.find("#SortAsc_" + pm.Get("GetId")).show();
                              }
                          }else{
                              headerMenu.find("#SortDesc_" + pm.Get("GetId")).show();
                              headerMenu.find("#SortAsc_" + pm.Get("GetId")).show();
                          ...
                          if (showUnlock) {
                              headerMenu.find("#Lock_" +  pm.Get("GetId")).hide();
                              headerMenu.find("#Unlock_" +  pm.Get("GetId")).show();
                          }
                          else {
                              headerMenu.find("#Unlock_" +  pm.Get("GetId")).hide();
                              headerMenu.find("#Lock_" +  pm.Get("GetId")).show();
                          }
                          ...
                          headerMenu.off("keydown").on('keydown', 'li',function(event,ui) {
                                ...
                                else if (event.keyCode  === $.ui.keyCode.LEFT || event.keyCode === $.ui.keyCode.UP) {
                          ...
                          headerMenu.off('click').on('click', 'li',function(event,ui) {
                              ...
                              parentEle = event.target.parentElement,
                              id = parentEle.getAttribute("id");
                              var SORTASC = "SortAsc_" + pm.Get("GetId"),
                                  SORTDESC = "SortDesc_" + pm.Get("GetId"),
                                  LOCK = "Lock_" + pm.Get("GetId"),
                                  UNLOCK = "Unlock_" + pm.Get("GetId");
                                  ...
                                  case SORTDESC:
                                      if (!(ele.classList.contains("ui-state-disabled"))) {
                                          ...
                                          if(id === SORTDESC){
                                              ...
                                              parentEle.setAttribute("rn", "sortColumn-desc");
                                              ascIcons.hide();
                                              descIcons.show();
                                          }else{
                                              GridOnSortCol.call(self, index, iCol, "asc");
                                              parentEle.setAttribute("rn", "sortColumn-asc");
                                              descIcons.hide();
                                              ascIcons.show();
                                          }
              ...
              function GridOnSelectRow(rowId, status, e) {
                ...
                function GridBeforeSelectRow(rowId, e) {
                    var multiSel,
                        ...
                        pm = self.GetPM(),
                        tdEle = $(e.target).closest("td");
                    if (pm.Get("IsInQueryMode")) {
                        var iCol = $.jgrid.getCellIndex(tdEle[0]),
                            grid = self.GetGrid(),
                            colVal = grid.jqGrid('getColProp', grid.getGridParam("colModel")[iCol].name);
                        if (colVal.frozen) {
                            tdEle = null;
                            return false; // prevent selection and so the editing of the cell for locked columns
                        }
                    }
                    ...
                    if (pm.Get("MultiSelectColumn")) {
                        ...
                        if (activeEl && (activeEl.attr("name") === pm.Get("GetPlaceholder") + pm.Get("MultiSelectColumn"))) {
                            ...
                            multiSel = activeEl;
                            var recordLength = Math.min(Number(pm.Get("GetRowListRowCount")), Number(pm.Get("RecordSetLength"))),
                                uiSelectedRecArr = pm.Get("GetRowsSelectedArray").slice(0, recordLength);
                            if ((bctrlsel && uiSelectedRecArr.indexOf(true) !== -1) || pm.Get("MultiSelectMode")) {
                                e.ctrlKey = true;
                            }
                        }
                    }
                    ...
                    var ret = self.OnRowSelect(rowId, e.ctrlKey || e.metaKey, e.shiftKey); //Important
                ...
                function refreshIscroll() {
                    if (!SiebelAppFacade.DecisionManager.IsTouch()) {
                        var grid = this.GetGrid(),
                        iscroll = grid.data("Iscroll");
                        grid.parent().css("height", grid.height() - 1);
                        setTimeout(function () {
                            if (iscroll) {
                                iscroll.refresh();
                            }
              ...
              function GridAfterEditCell(rowid, cellname, value, iRow, iCol) {
                  ...
                  if (self && self.length !== 0) {
                      if(Number(self.GetSelectedRow()) !== iRow){
                          self.GetGrid().jqGrid('resetEditCell');
                          setTimeout(function(){
                              self.GetGrid().jqGrid('editCell', Number(self.GetSelectedRow()), iCol, true);
                          }, 0);
                          return;
                      }
                      ...
                      if (grid.jqGrid("getGridParam", "colModel")[iCol].isRequired) {
                          ...
                          case checkbox:
                              ...
                              .bind("keydown", { ctx: self, row: iRow, col: iCol }, function (evt) {
                                  var colModel = evt.data.ctx.GetGrid().jqGrid("getGridParam", "colModel"),
                                      len = colModel.length;
                                  var treeReader = evt.data.ctx.GetGrid().jqGrid("getGridParam", "treeReader"),
                                      len1 = Object.keys(treeReader).length;
                                  var treeGrid = evt.data.ctx.GetGrid().jqGrid("getGridParam", "treeGrid");
                                      

                                  if (evt.data.col === 0 || (evt.data.col === 1 && !colModel[0].editable)) {// multiselect column
                                      MoveToPreviousRow.call(evt.data.ctx, evt);
                                  }

                                  var modifiedLength = treeGrid ? len - len1 : len;
                                  if( evt.data.col == (modifiedLength - 1) || (evt.data.col === (modifiedLength - 2) && !colModel[ modifiedLength - 1].editable )) {
                      ...
                      if (control.GetUIType() !== checkbox) {
                          ...
                          ctrlEl.addClass("siebui-align-" + control.GetJustification());
                      }
                      ...
                      if (SiebelApp.S_App.GetAccessibilityEnhanced() !== undefined && utils.IsTrue(SiebelApp.S_App.GetAccessibilityEnhanced())) {
                          ...
                          if (parentDescBy) {
                              parentDescBy = parentDescBy.trim();
                          }
                          if (controlType === checkbox) {
              ...
              function GridAfterSaveCell(rowid, cellname, value, iRow, iCol) {
                  ...
                  if (value === " " && el.data('resetEmptyValue')) {
                      self.SetCellValue(rowid, cellname, '',true);
              ...
              function GridBeforeSaveCell(rowid, cellname, value, iRow, iCol) {
                  ...
                  if (iCol === gridCtrl.jqGrid("getGridParam", "expColInd") && self.GetPM().Get("HasHierarchy")) {
                      iRow = Number(iRow);
              ...
              function GridAfterRestoreCell(rowid, value, iRow, iCol) {
                  ...
                  if (iCol === jqGridCtrl.jqGrid("getGridParam", "expColInd") && self.GetPM().Get("HasHierarchy")) {
                      iRow = Number(iRow);
              ...
              function GridResizeStop(width, index) {
				  ...
				  $(this).setColProp(cm[index].name, {'widthOrg': width});
                  cm = null;
              ...
              JQGridRenderer.prototype.SetCellValue = function (rowId, fieldName, newValue, bResetEmptyValue) {
                  var retValue = false,
                      pm = this.GetPM();
                  if (pm.Get("MultiSelectColumn") !== fieldName) {
                      ...
                      if (jqGridCtrl.length) {
                          ...
                          if (control) {
                             ...
                             */
                              if (pm.Get("IsInQueryMode")) {
                                  ...
                                  rowid = bResetEmptyValue ? rowId : pm.Get("GetSelection") + 1;
                              }
                          ...
                          if (!pm.Get("UpdateUI")) {
                              var control = pm.ExecuteMethod("GetControl", fieldName);
                              if (control) {
                                  this.SetControlValue(control, newValue, rowId);
                              }
                          }
                      }
                  }
              ...
              JQGridRenderer.prototype.FocusFirstControl = function () {
                  ...
                  var listofColumns = this.GetPM().Get("ListOfColumns");
                  var colModel = this.GetGrid().jqGrid("getGridParam", "colModel");
                  if (!colModel || (jqGridCtrl.closest("#sieb-ui-popup-mvg-selected").length && !this.GetPM().Get("IsInQueryMode"))) {
              ...
              JQGridRenderer.prototype.SetFocusToControl = function (name, setFocus) {
                  ...
                  var currentRow = Number(jqGridCtrl.jqGrid('getGridParam', 'selrow'));
                  if (currentRow === 0) {
                      if (name && !this.GetPM().ExecuteMethod("GetListCol", name)) {
                          return SiebelAppFacade.JQGridRenderer.superclass.SetFocusToControl.call(this, name, setFocus);
                      }
                      return;
                  }
                                  ...
                                  if (listOfColumns[i].name === map_col[key]) {
                                      if (listOfColumns[i].name === name) {
                                          ...
                                          if (this.GetPM().Get("SyncNavigation")) {
                                              setFocusToCtrl.call(this);
                                              this.GetPM().SetProperty("SyncNavigation", false);
                                          }
                                          else {
                                              var funcSetTimeout = setTimeout(setFocusToCtrl, 10);
                          ...
                          if (isMultiRowForm) {
                              ...
                              for (var i = 0; i < rowCount && i < len; i++) {
                                ...
                                if (selArray) {
                                    ...
                                    if (cellname && pm.Get("MultiSelectMode")) {
                                ...
                                SiebelAppFacade.JQGridRenderer.superclass.ShowSelection.call(this, (index && jqGridCtrl.length > 0) || undefined,pm.Get("BaseAppletControls"));
                        }
                  }
              };
              ...
              JQGridRenderer.prototype.UpdateUIControls = function () {
                  SiebelAppFacade.JQGridRenderer.superclass.UpdateUIControls.call(this, arguments[0], this.GetGrid().length > 0 ? this.GetPM().Get("BaseAppletControls") : arguments[1]);
              };
              ...
              JQGridRenderer.prototype.UpdateSelectedRow = function () {
                  ...
                  this.GetPM().SetProperty("ColHeader", "");
                  this.GetPM().SetProperty("RowHeader", "");
              ...
              JQGridRenderer.prototype.OnPagination = function (title) {
                  ...
                  switch (title) {
                      ...
                      case "last_pager": // Next Set of Record
                          direction = siebConsts.get("PAG_LAST_SET");
                          break;
                      case "prev_pager":
                          direction = siebConsts.get("PAG_PREV_SET");
                          break;
                      case "first_pager":
                          direction = siebConsts.get("PAG_FIRST_SET");
                          break;
                      case "scroll_up":
                  ...
                  setTimeout(function () {
                      if (!that.GetPM().OnControlEvent(siebConsts.get("PHYEVENT_VSCROLL_LIST"), direction)) {
                         // Error Handling ?
                      }
                  },0);
              };
              ...
              JQGridRenderer.prototype.OnRowSelect = function (rowId, ctrlKey, shiftKey) {
                  var bReturn = false,
                      ...
                      pm = this.GetPM(),
                      ret;
                  var isActive = pm.Get("IsActive");
                  if (!isActive) {
                      pm.ExecuteMethod("SetPrsrvControl", pm.Get("GetActiveControl"));
                      isActive = pm.OnControlEvent(siebConsts.get("PHYEVENT_APPLET_FOCUS"));
                  }
                  if (isActive) {
                    if (!ctrlKey && !shiftKey
                            && (Number(rowId) === Number(this.GetSelectedRow()) || Number(rowId) === Number(pm.Get("GetSelection")) + 1)
                            && this.GetSelectedRow('all')
    ...
    SiebelAppFacade.GridColumnHelper = function () {
        ...
        this.GetModString = function (inStr) {
            ...
            inStr = inStr.replace(/title/i, 'title');// jqgrid bug
