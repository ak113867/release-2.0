  ...
  function () {
   SiebelAppFacade.VHACustomerDashboardRecentActivitiesListAppletPR = (function () {
	...
	//var recentActtable = document.querySelector("table[summary='Recent activities']");
			...
			/* NBA Offer Code*/
			noData("VHA Customer Dashboard NBA Offers List Applet","Next Best Activity (NBA) Offers","nooffer","Nodata","No available offers","7",".vha-img-applet5","pagenavigationOffer"); //CLMNBA-2891,CLMNBA-2892 - Column changed from 5 to 7
			/* NBA Offer Code*/
			...
			function noData(appletName,summary,classname,Idval,msg,colCount,appid,pagenavigationID){
				...
				//R 1.2 defect fix 
				if( appid == ".vha-img-applet6")
				{
					$("table[summary='"+summary+"']").removeClass('displaynone');
					$('#nopubNotesspan').remove();
				}
				//R 1.2 defect fix 
				if (recCount == 0)
					{
						...
						//actboday= $("table[summary='Recent activities']").find('tbody');	
							...
							{						
								...
								{
									$('#nopubNotesspan').remove();
									$("table[summary='"+summary+"']").addClass('displaynone');
									$(appid).find('.siebui-applet-title').after('<span id="nopubNotesspan">'+msg+'</span>');
								}
								//R 1.2 defect fix 
							}
			...
			//R 1.2 defect fix 
			if(SiebelApp.S_App.GetActiveView().GetApplet("VHA Customer Dashboard Public Notes Applet").GetPModel().Get("GetRecordSet").length == 0 )
			{
				$(".vha-img-applet6").find('.ui-jqgrid-pager').addClass('displaynone');
				$(".vha-img-applet6").find('.ui-jqgrid-bdiv').addClass("noPublicNotes")
			}
			else{
				$(".vha-img-applet6").find('.ui-jqgrid-pager').removeClass('displaynone');
				$(".vha-img-applet6").find('.ui-jqgrid-bdiv').removeClass('noPublicNotes');
			}
			if(SiebelApp.S_App.GetActiveView().GetApplet("VHA Customer Dashboard Service Requests Applet").GetPModel().Get("GetRecordSet").length == 0 )
			{
				$(".vha-img-applet8").find('.ui-jqgrid-pager').addClass('displaynone');
				$(".vha-img-applet8").find('.ui-jqgrid-bdiv').addClass("noServiceReqdata")
			}
			else{
				$(".vha-img-applet8").find('.ui-jqgrid-pager').removeClass('displaynone');
				$(".vha-img-applet8").find('.ui-jqgrid-bdiv').removeClass('noServiceReqdata');
			}
			//R 1.2 defect fix 
						...
						if($("#s_"+AssetAppletId+"_div").find(".siebui-row-counter").text().slice(-1) != "+" && SiebelApp.S_App.GetActiveView().GetApplet("VHA Customer Dashboard Assets Billing Applet").GetRecordSet().length < 5) // Added on March32025 
							{
	...
	$("td[aria-roledescription='Expand']").off().on('click', '.plus-button', function(e) {
	  ...
	  var Inputs = SiebelApp.S_App.NewPropertySet();
		...
		<div class="actrow row1">\
			<span class="actlabel actForlable">Activity for</span>\
			<span class="actvalue activityfor">'+activityFor+'</span>\
		</div>\
		<div class="actrow row2">\
			<span class="actlabel createdBylable">Created by</span>\
			<span class="actvalue createdby">'+createdby+'</span>\
		</div>\
		<div class="actrow row3">\
			<span class="actlabel commentslable">Comments</span>\
		</div>\
		<div class="actrow row4">\
			<span class="actvalueCom commnents">'+commnents+'</span>\
		</div>\
		</div>');
