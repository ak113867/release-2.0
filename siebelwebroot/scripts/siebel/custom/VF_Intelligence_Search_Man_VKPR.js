if (typeof(SiebelAppFacade.VF_Intelligence_Search_Man_VKPR) === "undefined") {
    SiebelJS.Namespace("SiebelAppFacade.VF_Intelligence_Search_Man_VKPR");
    define("siebel/custom/VF_Intelligence_Search_Man_VKPR", ["order!siebel/phyrenderer", "order!siebel/custom/VHAAppUtilities"], function () {
        SiebelAppFacade.VF_Intelligence_Search_Man_VKPR = (function () {
 $('[aria-label="RAF Status"]').addClass("VFDisplayNone");

            var siebConsts = SiebelJS.Dependency("SiebelApp.Constants");
            function VF_Intelligence_Search_Man_VKPR(pm) {
                SiebelAppFacade.VF_Intelligence_Search_Man_VKPR.superclass.constructor.call(this, pm);
                this.GetPM().AttachPostProxyExecuteBinding("GetURL", VHACovergaeCheck, {
                    scope: this,
                    sequence: false
                });
            }
            SiebelJS.Extend(VF_Intelligence_Search_Man_VKPR, SiebelAppFacade.PhysicalRenderer);            
			var MapShed = {
                Mobile: {
                    m5G: {
                        indoor: false,
            ...
            VF_Intelligence_Search_Man_VKPR.prototype.Init = function () {
                SiebelAppFacade.VF_Intelligence_Search_Man_VKPR.superclass.Init.apply(this, arguments);
                var this_t = this;
				var sMResp = "";
				var stext = "";
                ...
                function OnFieldChange(control, value) {
					...
					if (control.GetName() == "Address Status") {
                        $('[name=' + this.GetPM().Get("GetControls")["Manual Service Address"].GetInputName() + ']').closest('tr').find('[aria-label="Manual Service Address"]').parent().find('div').remove();
                        var sStatusClass = (value == "Valid") ? "Class_Active" : ((value == "Not Valid") ? "Class_Inactive" : "");
                        if (sStatusClass != "") {
                            $('[name=' + this.GetPM().Get("GetControls")["Manual Service Address"].GetInputName() + ']').closest('tr').find('[aria-label="Manual Service Address"]').addClass("VFDisplayNone").parent().append("<div><span class=' dot_" + sStatusClass + "'></span> <span  class=' AddressStatusStyle " + sStatusClass + "'>" + value + "</span></div>");
                        }
                    }
                ...
                this.GetPM().AttachPMBinding("PostExecute", function (method_name, input_property_set, output_property_set) {
                    if (method_name == "PostChanges" && input_property_set.GetProperty("SWEPOC") == "Receive Unknown Flag") {
                        ...
                        var ShipingApplet = SiebelApp.S_App.GetActiveView().GetApplet("VF Order Line Item List Applet Summary - TBUI MSO");
            ...
            VF_Intelligence_Search_Man_VKPR.prototype.ShowUI = function () {
                SiebelAppFacade.VF_Intelligence_Search_Man_VKPR.superclass.ShowUI.call(this);
                var this_t = this;
                ...
                //$('[aria-labelledby*="WSDL_"]').not('[aria-labelledby=' + $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').attr('aria-labelledby') + '], [aria-labelledby="WSDL_Shipment_Address_Label"]').closest('td').parent().css("display", "none");
				...
				$('[aria-label="Customer Type Form Applet:Coverage Check"]').addClass("VFDisplayNone");
                ...
                setTimeout(function () {
                    ...
                    if (sAppltName == "VHA Capture Customer Manual Address FWA Details Person TBUI") {
                        $('[name=' + this_t.GetPM().Get("GetControls")["Manual Service Address"].GetInputName() + ']').closest('tr').find('[aria-labelledby=' + $('[name=' + this_t.GetPM().Get("GetControls")["Manual Shipment Address"].GetInputName() + ']').attr('aria-labelledby') + ']').parent().append('<div class="ChevronActive VHAShipmentChevron" id="ShipCollapse_up"></div>');
                        $('[name=' + this_t.GetPM().Get("GetControls")["Map URL Field"].GetInputName() + ']').parent().hide();
						//vasavi added for moconsq
						/*$('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').closest('tr').find('[aria-labelledby=' + $('[name=' + this_t.GetPM().Get("GetControls")["Manual Shipment Address"].GetInputName() + ']').attr('aria-labelledby') + ']').parent().append('<div class="ChevronActive VHAShipmentChevron" id="ShipCollapse_up"></div>');
                        $('[name=' + this_t.GetPM().Get("GetControls")["Map URL Field"].GetInputName() + ']').parent().hide();*/
                    }
                    ...
                    if (sAppltName == "VHA Capture Customer Details Toggle Form Applet Person TBUI" || sAppltName == "VHA Capture Customer Details Form Applet FWA TBUI") {
                        //$('[aria-labelledby*="WSDL_"]').not('[aria-labelledby=' + $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').attr('aria-labelledby') + '], [aria-labelledby="WSDL_Shipment_Address_Label"]').closest('td').parent().css("display", "none");
                        var str1 = this_t.GetPM().Get("GetControls")["WSDL Service Address"] === undefined ? "" : $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').attr('aria-labelledby');
						...
						var str3 = this_t.GetPM().Get("GetControls")["WSDL Service Address"] === undefined ? "" : $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').attr('aria-labelledby');//vasavi added for moconsq
                        var strF;
                        if (str1 != "") {
                            strF = '[aria-labelledby=' + str1 + ']';
                        }
						//vasavi added below if foe moconsq
						if (str3 != "") {
                            strF = '[aria-labelledby=' + str3 + ']';
                        }
                        if (str2 != "") {
                            if (strF != "") {
                        ...
                        $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').parent().find(".siebui-icon-pick").remove();
						$('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').parent().find(".siebui-icon-pick").remove();//vasavi added for moconsq
                    }
                    ...
                    $.each(this_t.GetPM().Get("GetControls"), function (controlName, controlObj) {
                        ...
                        case "Cal Full Address":
                            ...
                            break;
                        case "Manual Service Address":
                            $('[name=' + controlObj.GetInputName() + ']').css("width", "240px");
                            break;
                        case "WSDL Service Address":
                            $('[name=' + controlObj.GetInputName() + ']').closest('tr').find('[aria-label="FWA Address Status"]').addClass("VFDisplayNone");
                            var SAddressStatus = $('[name=' + controlObj.GetInputName() + ']').closest('tr').find('[aria-label="FWA Address Status"]').val();
                            var sStatusClass = (SAddressStatus == "Valid") ? "Class_Active" : ((SAddressStatus == "Not Valid") ? "Class_Inactive" : "");
                            if (sStatusClass != "") {
                                $('[name=' + controlObj.GetInputName() + ']').closest('tr').find('[aria-label="FWA Address Status"]').addClass("VFDisplayNone").parent().append("<div><span class=' dot_" + sStatusClass + "'></span> <span class=' AddressStatusStyle " + sStatusClass + "'>" + SAddressStatus + "</span></div>");
                            }
                            break;
						case "Manual Service Address":
                    ...
                    //$('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').closest('tr').find('[aria-label="Address Status"]').addClass("VFDisplayNone");
                    ...
                    if ("" + sView == "VF Capture Customer Details – Postpay TBUI" || "" + sView == "VF Capture Exst Customer Details Postpay TBUI" || "" + sView == "VF Capture Customer Details Postpay UNISIM TBUI" || "" + sView == "VHA PosttoPre Capture Customer Details View" || "" + sView == "VF Capture Customer Details – Postpay FWA TBUI View") {
                    ...
                    //$('[aria-labelledby="Address_Selected_Label"]').css("width", "240px");
                    /*var SAddressStatus = $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').closest('tr').find('[aria-label="Address Status"]').val();
                    var sStatusClass = (SAddressStatus == "Valid") ? "Class_Active" : ((SAddressStatus == "Not Valid") ? "Class_Inactive" : "");
                    if (sStatusClass != "") {
                    $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').closest('tr').find('[aria-label="Address Status"]').addClass("VFDisplayNone").parent().append("<div><span class=' dot_" + sStatusClass + "'></span> <span class=' AddressStatusStyle " + sStatusClass + "'>" + SAddressStatus + "</span></div>");
                    }*/
				...
				//Vasavi added for moconsq
				$('[aria-label="Customer Type Form Applet:Get Address"]').on("click",function(){
					var sId = this_t.GetPM().Get("GetId");
					var street = $('[name=' + this_t.GetPM().Get("GetControls")["Street Type"].GetInputName() + ']').closest('tr').find('[aria-labelledby="Street_Type_Label_'+sId+'"]').val();
					var streetnum = $('[name=' + this_t.GetPM().Get("GetControls")["Street"].GetInputName() + ']').closest('tr').find('[aria-labelledby="Street_Label_'+sId+'"]').val()
					var streetaddr = $('[name=' + this_t.GetPM().Get("GetControls")["Street Address"].GetInputName() + ']').closest('tr').find('[aria-labelledby="Street_Address_Label_'+sId+'"]').val();
					var postcode = $('[name=' + this_t.GetPM().Get("GetControls")["Postal Code"].GetInputName() + ']').closest('tr').find('[aria-labelledby="Postal_Code_Label_'+sId+'"]').val();
					var state = $('[name=' + this_t.GetPM().Get("GetControls")["State"].GetInputName() + ']').closest('tr').find('[aria-labelledby="State_Label_'+sId+'"]').val();
					var town = $('[name=' + this_t.GetPM().Get("GetControls")["Suburb"].GetInputName() + ']').closest('tr').find('[aria-labelledby="Suburb_Label_'+sId+'"]').val();
					
					if(street == undefined || streetnum == undefined || streetaddr == undefined || postcode == undefined || state == undefined || town == undefined || street == "" || streetnum == "" || streetaddr == "" || postcode == "" || state == "" || town == "")
						alert("Enter all the required fields");
					else
					{
						stext = $('[aria-label="Address Selected"]').val();
						$('[aria-label="Manual Service Address"]').val(stext);
					}
				});
				if ("" + sView == "VF Capture Customer Details – Postpay FWA TBUI View" || "" + sView == "VF Capture Exst Customer Details – Postpay FWA TBUI View" || "" + sView =="VF Change Location Adress Details-Postpay FWA TBUI") {
					$('[aria-label="Manual Service Address"]').focus();
					this_t.GetPM().Get("GetControls")["Manual Service Address"] === undefined ? "" : $('[name=' + this_t.GetPM().Get("GetControls")["Manual Service Address"].GetInputName() + ']').autocomplete({
                        source: function (request, response) {
                            var Inputs = SiebelApp.S_App.NewPropertySet();
							Inputs.SetProperty("Address",$('[aria-label="Manual Service Address"]').val());
                            var sResp = VHAAppUtilities.CallBS("VHA FWA Store Pickup Reservation Service BS MOCN", "NewPSMA", Inputs, "");
                            var resultCov = [];
							sMResp = sResp;
			                for (let i = 1; i < sResp.childArray.length; i++) {
								var sAddr = new Object;
			                    sAddr.id = sResp.childArray[i].propArray.Id;
								sAddr.value = sResp.childArray[i].propArray.Addr;
								resultCov.push(sAddr);
							}
                            if (resultCov != false) {
                                response(resultCov);
                            } else {
                        ...
                        select: function (event, ui) {
                            $('.ccNwkpar').remove();
							var sAddrId = ui.item.id;
							var lat = "",lon ="";
							var leng = sMResp.childArray[0].childArray[1].childArray[0].childArray.length;
							for(let i=0;sMResp.childArray[0].childArray[1].childArray[0].childArray.length;i++)
							{
								var sRAddrId = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.addressId;
								if(sRAddrId == sAddrId)
								{
									lon = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[1].childArray[0].childArray[0].childArray[0].propArray.Value;
									lat = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[1].childArray[0].childArray[0].childArray[1].propArray.Value;
									
									var sStreet1 = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.streetNumber1;
                                    var sStreet2 = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.streetNumber2;
                                    var sStreet = (sStreet1 === undefined) ? "LOT" : (sStreet2 !== undefined) ? sStreet1 + "-" + sStreet2 : sStreet1;
									var sFloor = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.complexUnitTypeDescription;
									var sStreetAddress = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.streetName;
									var sStreetType = sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.streetTypeDescription;
									
                                   SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA Postal Code", sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.postcode);
									SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA City", sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.localityName);
									SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA State", sMResp.childArray[0].childArray[1].childArray[0].childArray[i].childArray[0].childArray[0].propArray.stateTerritory);
									if (sFloor != undefined || (sFloor != null  && sFloor != ""))
										SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA Address Floor Type", capitalize(sFloor));
									else
										SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA Address Floor Type", "");
									
									SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA Street Address 2", sStreet);
									
									if (sStreetAddress != undefined || (sStreetAddress != null  && sStreetAddress != ""))
										SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA Street Address", capitalize(sStreetAddress));
									if (sStreetType != undefined || (sStreetType != null  && sStreetType != ""))
										SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("FWA Address Street Type", capitalize(sStreetType));
							...
							//var lat = -33.85777653 ;
							//var lon = 151.20907086;
							var sArray = new Object;
							var sResp = [];
							sArray.lat = lat;
							sArray.lon = lon;
							sResp.push(sArray);
                            sResp.gnafId = ui.item.id;
                            //var sResp = VHAAppUtilities.getAddress(ui);
                            //if (sResp != false) {
                                /*var Inputs = SiebelApp.S_App.NewPropertySet();
                                Inputs.SetProperty("ViewName", SiebelApp.S_App.GetActiveView().GetName());
                                Inputs.SetProperty("Action", "UpdateTBCFWAAddress");
                                VHAAppUtilities.updateAddress(sResp, Inputs);*/
            ...
            VF_Intelligence_Search_Man_VKPR.prototype.BindEvents = function () {
                SiebelAppFacade.VF_Intelligence_Search_Man_VKPR.superclass.BindEvents.call(this);
				...
				/*$('[aria-label="Customer Type Form Applet:Coverage Check"]').on("click",function(){
						VHACovergaeCheck(sMResp,this_t);
                        var sCoverageCheckStatus = $('[aria-label="Coverage Check Status"]').val();
                        var sStatClass = (sCoverageCheckStatus == "Error") ? "Class_Inactive" : ((sCoverageCheckStatus == "Completed" || sCoverageCheckStatus == "Checked") ? "Class_Active" : ((sCoverageCheckStatus == "Submitted") ? "Submitted" : ""))
                        if (sStatClass != "") {
                            //$('[aria-label="Coverage Check Status"]').addClass("VFDisplayNone").parent().append("<div><span  class=' CoverageChckSts dot_" + sStatClass + "'></span> <span class=' AddressStatusStyle " + sStatClass + "'>" + sCoverageCheckStatus + "</span></div>");
                ...
                if (!(myApplet1 == undefined || myApplet1 == "") || !(myApplet6 == undefined || myApplet6 == "")) {
                    ...
                    $("#s_" + AppletId + "_div").delegate("#ShipCollapse_up", "click", function () {
                        ...
                        $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').closest('tr').find('[aria-labelledby=' + $('[name=' + this_t.GetPM().Get("GetControls")["Manual Shipment Address"].GetInputName() + ']').attr('aria-labelledby') + ']').parent().append('<div class="ChevronInactive VHAShipmentChevron" id="ShipCollapse_down"></div>');
            ...
            VF_Intelligence_Search_Man_VKPR.prototype.EndLife = function () {
                SiebelAppFacade.VF_Intelligence_Search_Man_VKPR.superclass.EndLife.apply(this, arguments);
            ...
            function VHACovergaeCheck(sResp, this_t) { // ??
                if (sResp == "") {
					...
					if (scustid != "")
					{					
                    var sOut = VHAAppUtilities.CallBS("VHA FWA Store Pickup Reservation Service BS MOCN", "GetAddressId", nInputs, "")
                ...
                } else if (sView == "VF Capture Exst Customer Details Postpay TBUI" || sView == "VF Capture Customer Details – Postpay TBUI" || "VF Capture Customer Details – Postpay FWA TBUI View") {
            ...
            function fCoverageCheck(sResp, this_t) {
                ...
                var nInputs = SiebelApp.S_App.NewPropertySet();
                nInputs.SetProperty("Service Name", "VHA FWA Store Pickup Reservation Service BS MOCN");
                nInputs.SetProperty("role", "VCS");
                nInputs.SetProperty("longitude", sResp[0].lon); //??
                nInputs.SetProperty("latitude", sResp[0].lat);
				nInputs.SetProperty("GNAFId", sResp.gnafId);
				var sView = SiebelApp.S_App.GetActiveView().GetName();
				if (sView == "VF Capture Customer Details Postpay UNISIM TBUI") {                    
				...
				var suiURL = SiebelApp.S_App.GetProfileAttr("URL_Service");
				...
				}else if (sView == "VF Capture Exst Customer Details Postpay TBUI" || sView == "VF Capture Customer Details – Postpay TBUI" ||  sView =="VF Capture Customer Details – Postpay FWA TBUI View" || sView == "VF Capture Exst Customer Details – Postpay FWA TBUI View" || sView =="VF Change Location Adress Details-Postpay FWA TBUI" ){			
            ...
            return VF_Intelligence_Search_Man_VKPR;
        }
            ());
        return "SiebelAppFacade.VF_Intelligence_Search_Man_VKPR";
    });
}
