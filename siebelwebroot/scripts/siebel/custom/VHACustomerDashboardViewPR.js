if (typeof(SiebelAppFacade.VHACustomerDashboardViewPR) === "undefined") {
 ...
 define("siebel/custom/VHACustomerDashboardViewPR", ["siebel/viewpr","order!siebel/phyrenderer", "order!siebel/custom/VHAAppUtilities","siebel/custom/bootstrap.min"],
  function () {
   SiebelAppFacade.VHACustomerDashboardViewPR = (function () {
	...
	console.log(cusAccId);
     ...
     var sLovFlg = VHAAppUtilities.GetPickListValues("", SearchString);
		...
		else
		{
		 ...
		 /* NBA Offer Code*/
		 //var pegaNBAResp = VHAAppUtilities.GetPickListValues("", "[List Of Values.Type]='VHA_REMOVE_NBA_RESP' AND [List Of Values.Value]= 'Remove NBA Responsibility' AND [List Of Values.Active]='Y'", {"All": "true"})[0].Name;
	...
	$('#lastinvoicelable').text("Last invoice"); //R 1.2 Defect Fix
	 var Objectid = SiebelApp.S_App.GetProfileAttr("VHA Cotact Primary Id");
		 ...
		 for (let i =0; i < AccRowId.length ; i++){
			if(AccRowId[i] != undefined)
			{
				if(i <= 5)
				{
				 ...
				 if(cusAccId == AccRowId[i])
				 {
					 accountLi.attr('drilldownAccountRowId',AccRowId[i]);
					 accountLi.attr('drilldownAccount',"Yes");
				 }
				 //drilldownAccount Code
				}
				if(i > 5)
				{
					...
					for (let i =6; i < AccRowId.length; i++){
						...
						if(cusAccId == AccRowId[i])
						 {
							 var drilldownAccount = "Yes";
						 }
						 else{
							 var drilldownAccount = "No";
						 }	
						...
						$('.accDropdown').append('<option value="'+accDetails+'" data-rowid="'+AccRowId[i]+'" accContactRole="'+AccRole[i]+'" drilldownAccount="'+drilldownAccount+'">'+accDetails+'</option>');
						//drilldownAccount Code
					}
					i = AccRowId.length;
				}
		...
		{
			 //document.getElementById("ViewLinkedContact").innerHTML = "View linked contact"; /*Commented as part of CM-3283*/
			 document.getElementById("ViewLinkedContact").innerHTML = "View "+linkedContactCount+" linked contacts"; /*Added as part of CM-3283*/
		}
		if(linkedContactCount == 0)
		{
			document.getElementById("ViewLinkedContact").innerHTML = "None";
			$('#ViewLinkedContact').addClass('viewlinkedContactsdisabled')
		}
		if(linkedContactCount >1)
		{
 ...
 //$('#ContactAction2').on('change',function(e){
  ...
  //$('#LaunchButton').on('click',function(e){
		...
		//var url = 'https://tpgtel-nbaa-prod1.pega.net/prweb/CRMAgentCustomAuth/app/sage-advisor_'+ranNumber; //PROD
		var url = 'https://tpgtel-nbaa-dt1.pega.net/prweb/CRMAgentCustomAuth/app/sage-advisor-drop1_'+ranNumber; //CRR
		//var url = 'https://tpgtel-nbaa-stg1.pega.net/prweb/CRMAgentCustomAuth/app/sage-advisor_'+ranNumber; //SIT
		  ...
		  /* NBA Offer Code*/
			...
			GotoviewwithrowId("Account Detail - Activities View",accRowid,"VHA SIS Account Entry Applet","Account","Account");
			 ...
			 $('#A5Role').parents('li').attr('acccontactrole',acccontactrole);
			  $('#A5Role').parents('li').find('button').addClass('vha-img-activeTab');
			 //$("#A1Name").parent().parent().attr('data-rowID',selectedRowID);
			 $("#A5Name").parent().parent().attr('data-rowID',selectedRowID); // Added Mar24 Prod observation 
...
function creditCheckData(crStatus,expiryDate,connections,equipmentLimit,creditStatusMessage){
		...
		$('.credit-check-container').addClass('displaynone');
		 ...
		 $("#status").append('<span class="info-icon"><img id="newinfoimg" src="images/custom/info_20_20.svg" alt="newinfo"><span class="tooltip-text">'+creditStatusMessage+'</span></span>');
	}
	...
	function oneBillingAccount(selVal)
	{
			...
			$(".billing-status").empty(); // Added Mar24 Prod observation 
	...
	function formatDate(date) {
	 ...
	 function changeAccDetails(accRowIDDetail){
		...
		{
			 //document.getElementById("ViewLinkedContact").innerHTML = "View linked contact"; /*Commented as part of CM-3283*/
			 document.getElementById("ViewLinkedContact").innerHTML = "View "+linkedContactCount+" linked contact"; /*Added as part of CM-3283*/
		}
		if(linkedContactCount == 0)
		{
			document.getElementById("ViewLinkedContact").innerHTML = "None";
		...
		if (ResponseFromAPI)
			{
							...
							for (let index = 0; index < filteredarraylen; index++) {
								...
								item.MSISDN= filteredarray[index].MSISDN;
								data.push(item);
							}
							console.log(data);	
							if(data.length>0)
				...
				{
					var row = $('<tr> class "nodata-recentAcc"><td id ="recentAcc-Nodata" colspan="7">No interactions in the past 90 days</td></tr>');	
					$("#vha-img-pega-offersTable tbody").html("");
...
function buildCoveragecheckUI(){			
			...
			<!-- Address section -->\
				...
				<div class="coverage Mobilecoverage">\
				    ...
				    <button class="btn vhascppbtn vha-sc-btn-border-clr" id="vha-sc-4G-IO"><span class="vha-sc-coverageStatus"></span><span id="vha-sc-4G-IO-mobile">4G - Indoor & Outdoor</span></button>\
					<button class="btn vhascppbtn vha-sc-btn-border-clr" id="vha-sc-5G-O"><span class="vha-sc-coverageStatus"></span><span id="vha-sc-5G-O-mobile">5G - Outdoor Only</span></button>\
					<button class="btn vhascppbtn vha-sc-btn-border-clr" id="vha-sc-5GNSA-NC"><span class="vha-sc-coverageStatus"></span><span id="vha-sc-5GNSA-NC-mobile">5G - NSA - No Coverage</span></button>\
				</div>\
				</div>\
				<div id ="fixedtab"class="Fixed fixedtabdisplaynone">\
					...
					<br>\
					<br>\
					<span class="mobiletittle" id= "fixedAvailable">Fixed connection is available</span>\
					<div class="fixedContainer">\
					<span class="mobiletittle">Service class</span><span class="svrcls mobilval" id="servClass"></span>\
					<span class="mobiletittle">Technology</span><span class="Tech mobilval" id="Techval"></span>\
					</div>\
				...
				<button class="btn vhascppbtn vha-sc-btn-border-clr" id="vha-sc-5G-NA"><span class="vha-sc-coverageStatus"></span><span id="vha-sc-5G-home-status">5G - Not Available</span></button>\
				<button class="btn vhascppbtn vha-sc-btn-border-clr" id="vha-sc-5GNSA-NA"><span class="vha-sc-coverageStatus"></span><span id="vha-sc-5GNSA-home-status">5G - NSA - Not Available</span></button>\
				</div>\
				</div>\
			<!--******************NBN covearage End***********************************  -->');
}
