			...
			SiebelAppFacade.VF_Intelligence_Search_FBB_PR = (function () {
					...
					setTimeout(function () {
						...
						if (!($("#" + controls["Check NBN Address"].GetInputName() + "_Ctrl").hasClass("VFDisplayNone"))) {
							...
							if(SiebelApp.S_App.GetProfileAttr("ManageServicesSubType") == "Change Technology")
							{
								...
								$('input[aria-labelledby*="VHA_NBN_Retrieve_Plans_Label"]').hide();
								$('[aria-label = "Retrieve NBN Business Plans Only?"]').hide();

							}
							//$('[aria-labelledby="Cal_Full_Address_Label"]').parent().parent().parent().hide();
							//Chitra Added for upgarde OUI SQ availablity change
							$('#NBN_Address_Information_Label_' + t_this.GetPM().Get("GetId")).parent().parent().parent().parent().parent().parent().before("<div class = 'VHANBNAddreSection'><div class='VHANBNPageLabel VHANBNAddressLabel'>Fixed Address Information</div><div class='VHANBNPageLabel VHANBNAddressSearchLabel'>Address Search Results</div></div>");
							...
							if ("" + sView == "VHA NBN Site Qualification Postpay TBUI") {
								...
								TriggerNBNAddress();
									/* var AssetAppletId = SiebelApp.S_App.GetActiveView().GetApplet("VHA SQ Service List Applet TBUI").GetPModel().Get("GetFullId");
									 if($('#s_' + AssetAppletId + "_div").length != 0)ss
									 {
								...
								var NBNcheckbox = $('input[aria-label="NBN"]');
									NBNcheckbox.on("change",function(){
										if(this.checked){
											...
											SiebelApp.S_App.SetProfileAttr("OverrideNetworkFlag","Y");
											SiebelApp.S_App.SetProfileAttr("OverrideNetworkSQFlag","Y");
				...
				function TriggerNBNAddress() {
					...
					if (service) {
						...
						var sFreeformaddressline1 = resultSet.GetProperty("Freeformaddressline1");
						var sLotNumber = resultSet.GetProperty("Lot Number");
						var sPlanNumber = resultSet.GetProperty("Plan Number");
						var sUnitType1 = resultSet.GetProperty("Unit Type");
						var sUnitNumber1 = resultSet.GetProperty("Unit Number");						
						var sSecondarySiteBuildingName = resultSet.GetProperty("Secondary Site Building Name");
						var sSecondaryRoadNumber1 = resultSet.GetProperty("Secondary Road Number1");
						var sSecondaryRoadNumber2 = resultSet.GetProperty("Secondary Road Number2");
						var sSecondaryRoadName = resultSet.GetProperty("Secondary Road Name");
						var sSecondaryRoadTypeCode = resultSet.GetProperty("Secondary Road Type Code");
						var sSecondaryRoadSuffixCode = resultSet.GetProperty("Secondary Road Suffix Code");
						var sDPID = resultSet.GetProperty("WSDL DPID");
						var sLatitude = resultSet.GetProperty("WSDL Latitude");
						var sLongitude = resultSet.GetProperty("WSDL Longitude");
						...
						/*sIntCallInputs.SetProperty("longitude", psmaResponce?.address?.geometry?.coordinates?.Length > 0 ? psmaResponce.address.geometry.coordinates[0] : "");
						sIntCallInputs.SetProperty("latitude", psmaResponce?.address?.geometry?.coordinates?.Length > 1 ? psmaResponce.address.geometry.coordinates[1] : "");
						sIntCallInputs.SetProperty("GnafPid", psmaResponce?.address?.properties?.address_identifier ?? "");*/
						...
						//Expiring Session
						var sSessionId = SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").GetFieldValue("Id");
