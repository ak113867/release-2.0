 ...
 define("siebel/custom/VFOneSQCoverageCheckPR", ["siebel/phyrenderer"],
  function () {
   SiebelAppFacade.VFOneSQCoverageCheckPR = (function () {
	...
	//Raj V Added the below code for NATCNewCR
	$('[aria-label="SQTransferServiceLineCount"]').parents('td').hide();
	$('[aria-label="SQTransferServiceLineCount"]').parents('td').prev().hide();
	$('[aria-label="Unknown AVC ID"]').parents('td').hide();
	$('[aria-label="Unknown AVC ID"]').parents('td').prev().hide();
	$('[aria-label="Unknown AVC ID"]').parent().hide();
	$('[id*="Unknown_AVC_ID_Label_6"]').hide();
	 ...
	 // Start -> Santhoshb added for NBN 27/02/2025
		...
		$('input[aria-labelledby*="Customer_Authorization_Taken_Label"]').click(function () {
			...
			if (confirmedNetwork == 'NBN' && customerCheck == 1 && (isUsed == 1 || nbnisUsed == 1) && isOrderAvail == 1) {
				...
				//Raj V Added the below code for NATCNewCR
				$('[aria-label="Unknown AVC ID"]').parents('td').show();
				$('[aria-label="Unknown AVC ID"]').parents('td').prev().show();
				$('[aria-label="Unknown AVC ID"]').parent().show();
				$('[id*="Unknown_AVC_ID_Label_6"]').show();
			} else {
				$('[aria-label="Access Service ID (AVC ID)"]').parents('td').hide();
				$('[aria-label="Access Service ID (AVC ID)"]').parents('td').prev().hide();
	...
	//SBABU-Service list refresh issue						
	if (SiebelApp.S_App.GetProfileAttr("ManageServicesSubType") == "Change Technology" && SiebelApp.S_App.GetProfileAttr("OverrideNetworkSQFlag")!="Y")
	{
		var sTechnology = SiebelApp.S_App.GetActiveBusObj('Order Entry (Sales)').GetBusCompByName('VHA Site Qualification BC').GetFieldValue('Access Technology');
		if (sTechnology == "Fibre To The Curb" || sTechnology == "Fibre To The Building" || sTechnology == "Fibre To The Node")	
		{	
			var AssetAppletId = SiebelApp.S_App.GetActiveView().GetApplet("VHA SQ Service List Applet TBUI").GetPModel().Get("GetFullId");
			var AssetAppletIdPR =SiebelApp.S_App.GetActiveView().GetApplet("VHA SQ Service List PR Applet TBUI").GetPModel().Get("GetFullId");
			if($('#s_' + AssetAppletId + "_div").length != 0)
			{
				$('#s_' + AssetAppletId + "_div").addClass("VHADisplaynone").removeClass("VHADisplayblock");												
			}
			if($('#s_' + AssetAppletIdPR + "_div").length != 0)
			{
				$('#s_' + AssetAppletIdPR + "_div").addClass("VHADisplayblock").removeClass("VHADisplaynone");
			}			
		...
		else
		{
			...
			if($('#s_' + AssetAppletId + "_div").length != 0)
			{
				$('#s_' + AssetAppletId + "_div").addClass("VHADisplayblock").removeClass("VHADisplaynone");												
			}
			if($('#s_' + AssetAppletIdPR + "_div").length != 0)
			{
				$('#s_' + AssetAppletIdPR + "_div").addClass("VHADisplaynone").removeClass("VHADisplayblock");												
			}
		}
	}
	else{
			var AssetAppletId =SiebelApp.S_App.GetActiveView().GetApplet("VHA SQ Service List PR Applet TBUI").GetPModel().Get("GetFullId");
			if($('#s_' + AssetAppletId + "_div").length != 0)
			{									
				$('#s_' + AssetAppletId + "_div").addClass("VHADisplaynone").removeClass("VHADisplayblock");
				SiebelApp.S_App.SetProfileAttr("OverrideNetworkSQFlag","");				
			}							
	}//End					
				}
                //$() get cha
