if (typeof(SiebelAppFacade.VF_Intelligence_Search_VKPR) === "undefined") {
    SiebelJS.Namespace("SiebelAppFacade.VF_Intelligence_Search_VKPR");
    define("siebel/custom/VF_Intelligence_Search_VKPR", ["order!siebel/phyrenderer", "order!siebel/custom/VHAAppUtilities"], function () {
        SiebelAppFacade.VF_Intelligence_Search_VKPR = (function () {
 $('[aria-label="RAF Status"]').addClass("VFDisplayNone");

            var siebConsts = SiebelJS.Dependency("SiebelApp.Constants");
            function VF_Intelligence_Search_VKPR(pm) {
                SiebelAppFacade.VF_Intelligence_Search_VKPR.superclass.constructor.call(this, pm);
                this.GetPM().AttachPostProxyExecuteBinding("GetURL", VHACovergaeCheck, {
                    scope: this,
                    sequence: false
                });
            }
            SiebelJS.Extend(VF_Intelligence_Search_VKPR, SiebelAppFacade.PhysicalRenderer);            
			var MapShed = {
                Mobile: {
                    m5G: {
                        indoor: false,
            ...
            VF_Intelligence_Search_VKPR.prototype.Init = function () {
                SiebelAppFacade.VF_Intelligence_Search_VKPR.superclass.Init.apply(this, arguments);
                var this_t = this;
				var chResp = "";
				var gResp = "";
                ...
                function OnFieldChange(control, value) {
                    ...
                    //
                    // }
                    if (control.GetName() == "FWA Address Status Display" || control.GetName() == "FWA Address Status") {
                        $('[name=' + this.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').closest('tr').find('[aria-label="FWA Address Status"]').parent().find('div').remove();
                        var sStatusClass = (value == "Valid") ? "Class_Active" : ((value == "Not Valid") ? "Class_Inactive" : "");
                        if (sStatusClass != "") {
                            $('[name=' + this.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').closest('tr').find('[aria-label="FWA Address Status"]').addClass("VFDisplayNone").parent().append("<div><span class=' dot_" + sStatusClass + "'></span> <span  class=' AddressStatusStyle " + sStatusClass + "'>" + value + "</span></div>");
                        }
                    }
            ...
            VF_Intelligence_Search_VKPR.prototype.ShowUI = function () {
                SiebelAppFacade.VF_Intelligence_Search_VKPR.superclass.ShowUI.call(this);
                var this_t = this;
                var controls = this.GetPM().Get("GetControls");
				...
				var ManSer = SiebelApp.S_App.GetProfileAttr("ManageServicesSubType");
				if (ManSer!="Change NBN Address" && ManSer!="Change Location FWA")
				{
				$('[id*="From_Address_Label_1"]').closest('td').parent().addClass('displaynone');
				$('[id*="To_Address_Label"]').addClass('displaynone');
				$('[id*="End_Date_Label"]').closest('td').parent().addClass('displaynone');
				$('[id*="From_Address_Label"]').addClass('displaynone');
				$('[id*="Start_Date_Label"]').closest('td').parent().addClass('displaynone');
				}
                ...
                setTimeout(function () {
                    ...
                    /*if ("" + sView == "VF Capture Customer Details – Postpay TBUI" || "" + sView == "VF Capture Exst Customer Details Postpay TBUI" || "" + sView == "VF Capture Customer Details Postpay UNISIM TBUI" || "" + sView == "VHA PosttoPre Capture Customer Details View" ||  "" + sView == "VF Capture Customer Details – Postpay FWA TBUI View" ||  "" + sView == "VF Capture Exst Customer Details – Postpay FWA TBUI View" || "" + sView == "VF Change Location Adress Details-Postpay FWA TBUI" ) {
					...
					/*var sFlag = SiebelApp.S_App.GetProfileAttr("InvokeFWAConnect");
					if(sView == "VF Capture Customer Details – Postpay FWA TBUI View" && sFlag == "Yes")
					{
							fCoverageCheck("", this_t)
					}*/
					
					if ("" + sView == "VF Change Location Adress Details-Postpay FWA TBUI" || "" + sView == "VF Capture Customer Details – Postpay FWA TBUI View" || "" + sView == "VF Capture Exst Customer Details – Postpay FWA TBUI View") {
						var sFWAOldAddr = SiebelApp.S_App.GetProfileAttr("FWAOldAddrId");
                        ...
                        Inputs.SetProperty("Service Name", "VHA FWA Service BS");
                        Inputs.SetProperty("Method Name", "GetCoverageDetails");
						Inputs.SetProperty("FWAOldAddrId", sFWAOldAddr);
						
						if(SiebelApp.S_App.GetProfileAttr("InvokeFWAConnect") == "Yes")
							Inputs.SetProperty("SessionId", SiebelApp.S_App.GetProfileAttr("CoTSession"));
						else
							Inputs.SetProperty("SessionId", vSessionId);
						
						var Output = sbs.InvokeMethod("Run Process", Inputs);
						var res = Output.GetChildByType("ResultSet");
                        var slat = res.GetProperty("Latitude");
						var slon = res.GetProperty("Longitude");
						slat = (slat != undefined ) ? slat : "";
						slon = (slon != undefined ) ? slon : "";
						if(slat != "" && slon != "")
						{
						...
						{
							if("" + sView != "VF Capture Customer Details – Postpay FWA TBUI View")
								VHACovergaeCheck("",this_t);
							//if(SiebelApp.S_App.GetProfileAttr("ManageServicesSubType") == "Change Location FWA")
								//VHACovergaeCheck("",this_t);
						}
                        var sCoverageCheckStatus = $('[aria-label="Coverage Check Status"]').val();
                        var sStatClass = (sCoverageCheckStatus == "Error") ? "Class_Inactive" : ((sCoverageCheckStatus == "Completed" || sCoverageCheckStatus == "Checked") ? "Class_Active" : ((sCoverageCheckStatus == "Submitted") ? "Submitted" : ""))
					...
					if("" + sView == "VF Change Location Adress Details-Postpay FWA TBUI" )
					{
						chResp = this_t;
					}
                ...
                if (sView == "VF Prepay Customer and Address info View - TBUI" || sView == "VF Capture Exst Customer Details Postpay TBUI" || sView == "VF Capture Exst Customer Details – Postpay FWA TBUI View" || sView == "VF Change Location Adress Details-Postpay FWA TBUI") {
                ...
                if ("" + sView == "VF Capture Customer Details – Postpay TBUI" || "" + sView == "VF Capture Exst Customer Details Postpay TBUI" || "" + sView == "VF Capture Customer Details Postpay UNISIM TBUI" || "" + sView == "VHA PosttoPre Capture Customer Details View") {
                    ...
                    this_t.GetPM().Get("GetControls")["WSDL Service Address"] === undefined ? "" : $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').autocomplete({
                        source: function (request, response) {
                            var sResp = VHAAppUtilities.doSearchAddress(request, "physical");
                            if (sResp != false) {
                                response(sResp);
				...
				if ("" + sView == "VF Capture Customer Details – Postpay FWA TBUI View" || "" + sView == "VF Capture Exst Customer Details – Postpay FWA TBUI View" || "" + sView == "VF Change Location Adress Details-Postpay FWA TBUI") {
					this_t.GetPM().Get("GetControls")["WSDL Service Address"] === undefined ? "" : $('[name=' + this_t.GetPM().Get("GetControls")["WSDL Service Address"].GetInputName() + ']').autocomplete({
                        ...
                        select: function (event, ui) {
                            ...
                            if (sResp != false) {
                                var Inputs = SiebelApp.S_App.NewPropertySet();
								var sFlag = $('[aria-label="Service Address As Primary"]').prop('checked');
								if(sFlag == true)
									Inputs.SetProperty("PrimaryFWAFlg", "Y");
								else	
									Inputs.SetProperty("PrimaryFWAFlg", "N");
                                Inputs.SetProperty("ViewName", SiebelApp.S_App.GetActiveView().GetName());
			...
			VF_Intelligence_Search_VKPR.prototype.BindData = function (bRefresh) {
			SiebelAppFacade.VF_Intelligence_Search_VKPR.superclass.BindData.apply(this, arguments);
				if(SiebelApp.S_App.GetProfileAttr("TBUIPostExstCust") != "Y")
					$('[aria-label="Service Address As Primary"]').prop('checked', true);
			}
            VF_Intelligence_Search_VKPR.prototype.BindEvents = function () {
                SiebelAppFacade.VF_Intelligence_Search_VKPR.superclass.BindEvents.call(this);
            ...
            VF_Intelligence_Search_VKPR.prototype.EndLife = function () {
                SiebelAppFacade.VF_Intelligence_Search_VKPR.superclass.EndLife.apply(this, arguments);
            ...
            function VHACovergaeCheck(sResp, this_t) { // ??
				...
				if (sMapURL =="")
				sMapURL = SiebelApp.S_App.GetProfileAttr("URL_Service");
            ...
            function fCoverageCheck(sResp, this_t) {
				...
				/*if(sView == "VF Capture Customer Details – Postpay FWA TBUI View" && sFlag == "Yes")
				{
					var vSessionId = SiebelApp.S_App.GetProfileAttr("CoTSession");
					var sbs = SiebelApp.S_App.GetService("VF BS Process Manager");
					var nInputs = SiebelApp.S_App.NewPropertySet();
				...
				//else
				//{
					SiebelApp.S_App.SetProfileAttr("Longitude",sResp.address.geometry.coordinates[0]);
					SiebelApp.S_App.SetProfileAttr("Latitude",sResp.address.geometry.coordinates[1]);
					...
					var vSessionId = SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details UNISIM TBC").GetFieldValue("Id");
						...
						/*
						if(sFlag == "Yes")//Vasavi added for Imagine
							var vSessionId = SiebelApp.S_App.GetProfileAttr("CoTSession");
						else
						var vSessionId = SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").GetFieldValue("Id");
						*/
				...
				//}
                $('[aria-label="Coverage Checked"]').prop("checked", true);
				SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").SetFieldValue("VHA FWA Coverage Flag","Y");
				...
				//SiebelApp.S_App.GetActiveBusObj().GetBusCompByName("VF Capture Customer Details TBC").WriteRecord();
				}								
                ...
                arrSite.forEach(function (item1, index) {
					...
					if(sView == "VF Capture Customer Details – Postpay FWA TBUI View" && sFlag == "Yes")
						hdrdiv = hdrdiv + '<div class="ccNwkchdmain"><div class="ccNwkchd">';
                    else
            ...
            return VF_Intelligence_Search_VKPR;
        }
            ());
        return "SiebelAppFacade.VF_Intelligence_Search_VKPR";
    });
}
