      ...
      SiebelAppFacade.VHAInviteContactPR = (function () {
       ...
       VHAInviteContactPR.prototype.ShowUI = function () {
		...
		var datatemp = Outputs.childArray[0].propArray;
	      ...
	      if(datatemp.AccOrg === 'Kogan'){
			$('#vha-img-Inc-Nominate-Yes').prop('checked',true);
			$('#vha-img-invitecotact-mfc-radioYes').prop('checked',true);
			$('#vha-img-Inc-Nominate-No').prop('disabled',true);
			$('#vha-img-invitecotact-mfc-radioNo').prop('disabled',true);
			koganFlag = "Y";
		  }
		  else{
			$('#vha-img-Inc-Nominate-Yes').prop('checked',false);
			$('#vha-img-invitecotact-mfc-radioYes').prop('checked',false);
			$('#vha-img-Inc-Nominate-No').prop('disabled',false);
			$('#vha-img-invitecotact-mfc-radioNo').prop('disabled',false);
		  }
		...
		//var AppuserType = SiebelApp.S_App.GetProfileAttr("VHA User Type");
		    $('.vha-table-step5-retailUser thead tr th:nth-child(8)').hide();
						   ...
						   case 'flow3':
								if(GlobalInputs.propArray.NomRole === "Limited User" && limiteduserRecSel === "" ){
								 ...
								 $(".vha-img-invitecotact-step3").addClass("displaynone");
									//var AppuserType = SiebelApp.S_App.GetProfileAttr("VHA User Type");
								   if(TheApplication().GetProfileAttr("VHA User Type") == 'Care'){		 
									 $(".vha-fil-CareUser").removeClass("displaynone");
									$(".vha-fil-retailUser").addClass("displaynone");
									$(".asterisk-inv").addClass("displaynone");
									
								   }
								   else{
									$(".vha-fil-CareUser").addClass("displaynone");
									$(".vha-fil-retailUser").removeClass("displaynone");
									$(".asterisk-inv").removeClass("displaynone");
 ...
 // step6 authentication end
       ...
       VHAInviteContactPR.prototype.EndLife = function () {
		...
		function createAuthorisedTable(data){
		  ...
		  $('.vha-img-invitecotact-step3 td:empty').html('&nbsp;');
		}
		...
		function updateNomineeForm(item){
		   ...
		   $(".unverifiedbtn-step6").addClass("displaynone");
		     ...
		     if(datatemp.AccOrg === 'Kogan'){
			   PhnVerifyFlag_step6 = "Y";
			   $('#idSighted-step6,label[for="idSighted-step6"]').addClass("displaynone");
			   $('#inApp-step6,label[for="inApp-step6"]').addClass("displaynone");
			   $('#email-step6,label[for="email-step6"]').addClass("displaynone");
		     }
			if(EmailVerifyFlag_step6 === "Y"){	 
              $("#vha-step6-mail-tickmark").removeClass("displaynone");
			  $("#vha-step6-mail-traingexclm").addClass("displaynone");	
			...
			if (datatemp.AccOrg === "Kogan") {
				...
				$("#vha-step6-mail-tickmark").addClass("displaynone");
				//$("#vha-step6-mail-unverified").text("Unverified");
				$("#vha-step6-mail-unverified").addClass("displaynone");
			}
		...
		function createNomineTable(){
			...
			var usertype = TheApplication().GetProfileAttr("VHA User Type");
			var resulstlimit_lov = VHAAppUtilities.GetPickListValues("", "[List Of Values.Type]='VHA_IN_CON_SRC_LMT' AND [List Of Values.Name]= 'LIMIT' AND [List Of Values.Active]='Y'", {
				All: "true",
			})[0].Value;
			if (datatemp.length > 0 ){
				if(Number(Outputs.GetChildByType("ResultSet")?.childArray[1]?.propArray.NomCnt) <= Number(resulstlimit_lov) || usertype === 'Retail'){ // verify count
					var data=[];
					for (let index = 0; index < datatemp.length; index++) {
						var item ={};
					...
					$(".vha-table-step5-retailUser tbody").html('');
					//alert("Please enter date of birth to find appropriate contact profile since the matching contact profiles are more than 10.");
					alert(`Please enter date of birth to find appropriate contact profile since the matching contact profiles are more than ${Number(resulstlimit_lov)}.`);
				}
			}
			else{
			...
			$('.vha-img-invitecotact-step5 td:empty').html('&nbsp;');
		}
		...
		function resetMFA(){
			...
			//kogan search mfa reset
		function resetMFAStep6(){
			$("#vha-email-send-otp-parent-step6").addClass("displaynone");
			 $("#vha-idsight-send-otp-btn-step6").addClass("displaynone");
			 $("#vha-inapp-send-otp-btn-step6").addClass("displaynone");
			 $("#vha-phone-send-otp-parent-step6").addClass("displaynone");
			...
			$(".otp-status-step6").hide(); 
			$(".validated-status-step6").hide();
			$(".vha-img-invitecotact-step6").find("input[type='radio'], input[type='checkbox']").prop("checked", false);
